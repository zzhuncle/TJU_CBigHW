...
int main(int argc, char** argv)
{
	...
	if (test_choise[T_MATHCURVE] == '1') {
		...
		/* 画一条数学曲线，具体内容可自选（例：阿基米德螺线）
		   必做项 */
		   /* 选择的是雪花分型曲线，其中基准三角形的三个顶点以及雪花花瓣的长度以及边线的厚度以及花瓣的颜色都可以自行选择 */
		   /* 为方便操作，仅给出随机生成的曲线 */
		void hdc_draw_Mathematical_curve(double x1, double y1, double x2, double y2, double x3, double y3, int length, int thickness, const int RGB_value = INVALID_RGB);
		hdc_set_pencolor(255, 255, 255);
		hdc_draw_Mathematical_curve(win_width / 2, win_high / 2 - 100 - 150, win_width / 2, win_high + 86 - 150, win_width / 2 + 50,
			win_high + 86 - 150, 1, 2);
		hdc_cls();
		for (i = 0; i < 10; i++) {
			int x1, y1, x2, y2, x3, y3, length, thickness;//length为雪花花瓣长度，thickness为雪花花瓣边线厚度
			hdc_set_pencolor(rand() % 256, rand() % 256, rand() % 256);
			x1 = rand() % (win_width);
			y1 = rand() % (win_high);
			x2 = rand() % (win_width);
			...
			length = rand() % 4 + 2;
			thickness = rand() % 5 + 2;
			hdc_draw_Mathematical_curve(x1, y1, x2, y2, x3, y3, length, thickness);
			Sleep(500);
		}
		to_be_continued("snowflake绘制测试完成", STL_X, STL_Y);
	}
	...
	if (test_choise[T_CUSTOM] == '1') {
		...
		int hdc_draw_Doraemon(const int base_x=400, const int base_y=200);
		hdc_draw_Doraemon(400, 300);//以头部中间的坐标为基准值
	}
